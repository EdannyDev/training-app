...
const EditFAQ = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchFAQ = async () => {
        setLoading(true);
        try {
          const { data } = await axios.get(`https://backend-training-nni3.onrender.com/api/faqs/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://backend-training-nni3.onrender.com/api/faqs/${id}`, updatedData, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
