...
const ForgotPassword = () => {
  ...
  const handleForgotPassword = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://backend-training-y050.onrender.com/api/users/forgot-password', { email });
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      await axios.post('https://backend-training-y050.onrender.com/api/users/reset-password', { token: resetToken, newPassword });
      setShowModal(false);
      setEmail('');
      setNewPassword('');
      setConfirmPassword('');
