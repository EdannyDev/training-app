...
const ForgotPassword = () => {
  ...
  const handleForgotPassword = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://backend-training-nni3.onrender.com/api/users/forgot-password', { email, securityCode });
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      await axios.post('https://backend-training-nni3.onrender.com/api/users/reset-password', { token: resetToken, newPassword });
      setShowModal(false);
      setEmail('');
      setSecurityCode('');
      setNewPassword('');
