...
const EditTraining = () => {
  ...
  useEffect(() => {
    const fetchTraining = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://backend-training-nni3.onrender.com/api/trainings/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://backend-training-nni3.onrender.com/api/trainings/${id}`, updatedData, {
        headers: { Authorization: `Bearer ${token}` },
      });
