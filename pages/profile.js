...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put('https://backend-training-cs9o.onrender.com/api/users/profile', updatedData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const response = await axios.get('https://backend-training-cs9o.onrender.com/api/users/profile', {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setTimeout(async () => {
        try {
          await axios.delete('https://backend-training-cs9o.onrender.com/api/users/profile', {
            headers: { Authorization: `Bearer ${token}` },
          });
          localStorage.removeItem('token');
          router.push('/login');
