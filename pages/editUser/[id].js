...
const EditUser = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      axios.get(`https://backend-training-cs9o.onrender.com/api/users/list/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://backend-training-cs9o.onrender.com/api/users/update/${id}`, updatedData, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
