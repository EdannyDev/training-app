...
const TrainingTable = () => {
  ...
  const fetchTrainings = async () => {
    try {
      const { data } = await axios.get('https://backend-training-y050.onrender.com/api/training', {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`https://backend-training-y050.onrender.com/api/training/${selectedTraining._id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
